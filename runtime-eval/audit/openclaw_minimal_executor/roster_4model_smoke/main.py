...
def main() -> None:
    ...
    use_four_model_roster = (
        tournament.get("num_models") == 4
        and isinstance(roster_models, list)
        and len(roster_models) >= 4
    )
    if use_four_model_roster:
        models4 = []
        for idx, entry in enumerate(roster_models[:4]):
        ...
        pairings = list(combinations(models4, 2))
        latest_post_by_model_id: dict[str, Path] = {}

        for match_idx, (left_meta, right_meta) in enumerate(pairings, start=1):
            round_idx = match_idx
            ...
            left_codebase = (
                ...
                / tournament_name
                / left_id
                / f"codebase_play_{round_idx}_vs_{right_id}"
            )
            right_codebase = (
                Path("workspace/codebases")
                / tournament_name
                / right_id
                / f"codebase_play_{round_idx}_vs_{left_id}"
            )
            left_submission = (
                Path("workspace/submissions")
                / tournament_name
                / left_id
                / f"submission_{round_idx}_vs_{right_id}"
            )
            right_submission = (
                Path("workspace/submissions")
                / tournament_name
                / right_id
                / f"submission_{round_idx}_vs_{left_id}"
            )
            left_post = (
                Path("workspace/posts")
                / tournament_name
                / left_id
                / f"codebase_post_{round_idx}_vs_{right_id}"
            )
            right_post = (
                Path("workspace/posts")
                / tournament_name
                / right_id
                / f"codebase_post_{round_idx}_vs_{left_id}"
            )
            ...
            feedback_payload = {
                "meta": {
                    ...
                    "protocol": "adaptive",
                    "round_idx": round_idx,
                    "match_id": f"{tournament['name']}_pair_{round_idx}",
                    "pairing_index": match_idx,
                    "pairing_left_model_id": left_id,
                    "pairing_right_model_id": right_id,
                    "regime": regime["name"],
        ...
        print("Created roster pairings codebase_play_t + submission_t + codebase_post_t artifacts in workspace/")

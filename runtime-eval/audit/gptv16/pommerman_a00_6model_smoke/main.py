...
def main() -> None:
    ...
    if use_generic_roster:
        ...
        def stable_pair_seed(pair_id: str) -> int:
            # Deterministic 31-bit seed so both seat-swap legs can reuse it.
            return int(hashlib.sha256(pair_id.encode("utf-8")).hexdigest()[:8], 16) & 0x7FFFFFFF
        ...
        for round_idx in range(1, tournament["num_rounds"] + 1):
            ...
            for match_slot, (left_meta, right_meta, seat_swap_leg) in enumerate(round_pairs, start=1):
                ...
                pair_seed = stable_pair_seed(pair_id)
        ...
        index_by_pair_leg: dict[tuple[str, int], int] = {}
        for idx, entry in enumerate(pairing_manifest_entries):
            index_by_pair_leg[(entry["pair_id"], entry["seat_swap_leg"])] = idx
        for entry in pairing_manifest_entries:
            other_leg = 2 if entry["seat_swap_leg"] == 1 else 1
            other_idx = index_by_pair_leg.get((entry["pair_id"], other_leg))

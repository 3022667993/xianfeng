...
def test_build_manifest_uses_stable_fallback_identities_and_leg_seats(tmp_path):
    ...
    _write_json(logs / "round_1" / "scorecard.json", {"left_score": 1, "right_score": 0})
    _write_json(logs / "round_2" / "scorecard.json", {"left_score": 0, "right_score": 1})
    ...
    assert leg1["seed_control_status"] == "not_recorded_in_current_smoke"

    assert leg2["leg"] == 2
    assert leg2["agent_A_seat"] == "right"
    assert leg2["agent_B_seat"] == "left"
    assert leg2["seed"] is None
    assert leg2["seed_control_status"] == "not_recorded_in_current_smoke"

...
def test_require_effective_change_accepts_intentional_no_change(tmp_path, monkeypatch):
    play, post = _mk_play(tmp_path)
    calls = []

    def fake_apply(*args, **kwargs):
        calls.append(bool(kwargs.get("retry_on_noop")))
        audit_path = kwargs["codebase_post_dir"] / "revision_audit.json"
        audit_path.write_text(
            json.dumps(
                {
                    "revisionDecision": "intentional_no_change",
                    "intentionalNoChange": True,
                    "noChangeRationale": "Current code is preferable on the feedback evidence.",
                    "noChangeEvidenceUsed": "feedback README and public scoreboard",
                }
            ),
            encoding="utf-8",
        )
    ...
    assert ok
    assert "intentionally left submission/main.py unchanged" in msg
    assert calls == [False]
    assert (post / "submission" / "main.py").read_text(encoding="utf-8").strip() == "AGGRESSION = 0"
...
def test_initial_synthesis_ignores_intentional_no_change_marker(tmp_path, monkeypatch):
    starter = tmp_path / "starter"
    post = tmp_path / "post"
    (starter / "submission").mkdir(parents=True, exist_ok=True)
    (starter / "submission" / "main.py").write_text("AGGRESSION = 0\n", encoding="utf-8")
    ...
    def fake_apply(*args, **kwargs):
        calls.append(bool(kwargs.get("retry_on_noop")))
        (kwargs["codebase_post_dir"] / "revision_audit.json").write_text(
            json.dumps(
                {
                    "revisionDecision": "intentional_no_change",
                    "intentionalNoChange": True,

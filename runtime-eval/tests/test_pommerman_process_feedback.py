...
def test_build_trajectory_summary_and_agent_feedback(tmp_path):
    ...
    assert summary["schema_version"] == "pommerman_process_feedback_v2"
    assert summary["seat_swap_summary"]["outcome_changed_under_swap"] is None
    ...
    assert fb["schema_version"] == "pommerman_agent_feedback_v2"
    assert fb["result_summary"]["wins"] == 1
    assert fb["source_files"]["trajectory_summary"].endswith("trajectory_summary.json")
    assert all("seat-swap instability" not in h for h in fb["next_round_hints"])
...
def test_stable_draw_draw_has_neutral_seat_swap_hint(tmp_path):
    match_dir = _mk_match(tmp_path)
    p = match_dir / "arena_result_match_a.json"

...
Config: `configs/models/openclaw_relay_current.yaml`

To replace the roster, edit only the entries in `configs/models/openclaw_relay_current.yaml`: `id`/`agent_id`, `provider_model`, and optional `stratum`. Keep `agent_id` values unique and use valid OpenClaw provider model strings.

Even model counts are supported by the active auto tournament configs:
- `num_models: auto`
- `matches_per_round: auto`
- `revision_subset_size: all`
- `revision_rounds: auto_before_final`
- full formal runs use `num_rounds: auto_full_double_rr`

Odd model counts are currently unsupported unless BYE scheduling is implemented.
...
The full-formal command below resolves `num_rounds`, `matches_per_round`, and revision coverage from the even-N active roster.
